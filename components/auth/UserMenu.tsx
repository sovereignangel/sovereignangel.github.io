    ...
    <div className="relative">
      ...
      {showMenu && (
        <>
          ...
          <div className="absolute right-0 top-full mt-1 bg-paper border border-rule rounded-sm shadow-sm z-20 min-w-[160px]">
            <button
              onClick={() => { setShowProof(true); setShowMenu(false) }}
              className="w-full text-left px-3 py-1.5 font-serif text-[11px] text-ink hover:bg-cream transition-colors flex items-center gap-1.5 border-b border-rule-light"
            >
              <span className="font-mono text-[10px] text-burgundy font-semibold">g*</span>
              <span>Reward</span>
            </button>
            <button
              ...
              className="w-full text-left px-3 py-1.5 font-serif text-[11px] text-ink hover:bg-cream transition-colors flex items-center gap-1.5 border-b border-rule-light"
            >
              <svg className="w-3 h-3 text-burgundy" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
            ...
            <div className="border-b border-rule-light">
              <span className="block px-3 pt-1.5 pb-0.5 font-mono text-[8px] text-ink-faint uppercase tracking-wider">Guidebooks</span>
              <button
                onClick={() => { setShowGuide('decisions'); setShowMenu(false) }}
                className="w-full text-left px-3 py-1 font-serif text-[10px] text-ink-muted hover:text-ink hover:bg-cream transition-colors"
              >
                Decisions
              </button>
              <button
              ...
              <button
                ...
                className="w-full text-left px-3 py-1 pb-1.5 font-serif text-[10px] text-ink-muted hover:text-ink hover:bg-cream transition-colors"
              >
                Cadence
              </button>
            </div>
            <Link
              ...
              className="block w-full text-left px-3 py-1.5 font-serif text-[11px] text-ink hover:bg-cream transition-colors no-underline border-b border-rule-light"
            >
              Settings
            </Link>
            <button
              onClick={() => { signOut(); setShowMenu(false) }}
              className="w-full text-left px-3 py-1.5 font-serif text-[11px] text-ink hover:bg-cream transition-colors"
            >
              Sign out
            </button>
          </div>
