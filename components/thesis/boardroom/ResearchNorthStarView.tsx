      ...
      <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr_260px] gap-3 h-full">
        ...
        <div className="flex flex-col gap-3 min-h-0">
          ...
          <div className="bg-white border border-rule rounded-sm p-3">
            ...
            <div className="space-y-1">
              <ReadingItem title="Reinforcement Learning: An Introduction" author="Sutton & Barto" status="core" />
              <ReadingItem title="Theoretical Neuroscience" author="Dayan & Abbott" status="core" />
              <ReadingItem title="Vision (Levels of Analysis)" author="David Marr" status="core" />
              <ReadingItem title="Empowerment — An Introduction" author="Klyubin, Polani, Nehaniv" status="queue" />
              <ReadingItem title="Curiosity-Driven RL" author="Pathak et al." status="queue" />
              <ReadingItem title="Active Inference" author="Friston et al." status="queue" />
              <ReadingItem title="Successor Representations" author="Dayan" status="queue" />
            </div>
          </div>
        </div>
      </div>
...
function PaperRow({ paper }: { paper: ResearchPaper }) {
  return (
    <a
      ...
      className="block px-1.5 py-1 rounded-sm hover:bg-cream/50 transition-colors group"
    >
...
function ReadingItem({ title, author, status }: { title: string; author: string; status: 'core' | 'queue' | 'done' }) {
  return (
    <div className="flex items-start gap-1.5">
      <span className={`mt-0.5 w-1.5 h-1.5 rounded-full shrink-0 ${
        status === 'core' ? 'bg-burgundy' : status === 'done' ? 'bg-green-ink' : 'bg-ink-faint'
      }`} />
      <div className="min-w-0">
        <div className="text-[10px] font-medium text-ink leading-tight">{title}</div>
        <div className="text-[8px] text-ink-muted">{author}</div>
      </div>
    </div>
  )
}
