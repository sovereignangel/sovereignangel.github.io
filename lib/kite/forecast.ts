/**
 * Kite wind forecast engine — region-agnostic.
 *
 * Fetches hourly wind from Open-Meteo (free, no API key) for each spot,
 * classifies every daylight hour against the rider's target band (12-30 kn,
 * gusts under 36, onshore or cross only), and finds the best contiguous
 * session window per spot per day.
 *
 * The rules are the rider's, not the region's — the same band applies in
 * Palanga, Brooklyn and Ceara. What varies per region is the spot list, the
 * timezone, and how spots are ranked against each other within a day.
 *
 * Region definitions live in `regions.ts`. `lithuania-spots.ts` re-exports
 * this module for the older callers (/exec, the wind-observe cron) that were
 * written when Lithuania was the only region.
 */
...
export interface KiteSpot {
  ...
  lat: number
  lon: number
  water: 'lagoon' | 'baltic' | 'bay' | 'ocean' | 'river'
  ...
  tagline: string
  note: string
  /** Flat ranking bonus/penalty when picking the day's spot — travel cost, mostly */
  priority?: number
  /** Extra bonus when the day's wind lands in this band — spot suits a strength */
  favorWhen?: { minKn?: number; maxKn?: number; bonus: number }
}
...
function spotForecastUrl(spot: KiteSpot, timezone: string): string {
  return (
 ...
 */
export async function fetchSpotHourly(spot: KiteSpot, timezone: string): Promise<Map<string, HourForecast[]>> {
  const res = await fetch(`${spotForecastUrl(spot, timezone)}&models=gfs_seamless`, { cache: 'no-store' })
...
export async function fetchSpotForecast(spot: KiteSpot, timezone: string): Promise<SpotForecast> {
  const base = spotForecastUrl(spot, timezone)
...
export async function fetchSpots(spots: KiteSpot[], timezone: string): Promise<SpotForecast[]> {
  return Promise.all(spots.map(s => fetchSpotForecast(s, timezone)))
}
...
/**
 * Days where GFS (the primary, Windguru's model) shows no window but the
...
/**
 * Within a day: prefer the spot's ideal (onshore) wind direction, then the
 * spot's own standing (`priority` for travel cost, `favorWhen` for the wind
 * band it suits best), then wind nearest 16 kn.
 */
function bySessionPriority(a: SessionPick, b: SessionPick): number {
  if (a.date !== b.date) return a.date.localeCompare(b.date)
  const score = (p: SessionPick) => {
    const kn = p.window.avgSpeedKn
    let s = -Math.abs(kn - 16)
    if (inSector(p.window.directionDeg, p.spot.onshoreSector)) s += 4
    s += p.spot.priority ?? 0
    const f = p.spot.favorWhen
    if (f && (f.minKn === undefined || kn >= f.minKn) && (f.maxKn === undefined || kn <= f.maxKn)) {
      s += f.bonus
    }
    return s
  }
