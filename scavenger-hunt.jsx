...
const TEAMS = [
  {
    ...
    accent: "#7c2d2d",
    members: ["Aidas", "Lori"],
    tagline: "Brooklyn energy, island execution",
  },
  {
    name: "BIRTHDAY VIBES",
    accent: "#8a6d2f",
  ...
  },
  {
    name: "TRIPLE THREAT",
    accent: "#2d4a6f",
    members: ["Alyssa", "Rodrigo", "Alberto"],
    tagline: "Numbers advantage, higher bar",
  },
];
...
function P({ children }) {
  return (
    <p style={{
      ...
      lineHeight: 1.5, color: "#5c5550", margin: "0 0 5px",
    }}>{children}</p>
  );
}
...
function RulesPage() {
  return (
    <div>
      ...
      <P>At <Mono>19:00</Mono> sharp, all teams return to the mansion. Late arrivals lose <Mono>5 pts/min</Mono>. Scores tallied. Proof reviewed. Champions crowned.</P>
      ...
      {TEAMS.map((t) => (
        <div key={t.name} style={{
          ...
          padding: "5px 12px", marginBottom: 5, background: `${t.accent}08`,
        }}>
          <div style={{
            fontFamily: "'Crimson Pro', Georgia, serif",
          ...
          <div style={{
            ...
            fontSize: 13, color: "#2a2522", fontWeight: 500,
          }}>{t.members.join(" & ")}</div>
          <div style={{
            fontFamily: "'Crimson Pro', Georgia, serif",
            fontSize: 11, color: "#9a928a", fontStyle: "italic",
          }}>{t.tagline}</div>
        </div>
      ))}
      ...
      {[
        ...
        "Bonus votes at 19:00: \"Most Creative Proof\" (+10) and \"Best Team Spirit\" (+10).",
        "The Kokomo Dance-Off will be judged by Carlos (concierge). His decision is final.",
      ].map((rule, i) => (
        <div key={i} style={{ display: "flex", gap: 8, marginBottom: 3 }}>
          <span style={{
            fontFamily: "'IBM Plex Mono', monospace", fontWeight: 600,
            color: "#7c2d2d", fontSize: 11, minWidth: 18,
          }}>{String(i + 1).padStart(2, "0")}</span>
          <span style={{
            fontFamily: "'Inter', sans-serif", fontSize: 12.5,
            color: "#5c5550", lineHeight: 1.4,
          }}>{rule}</span>
        </div>
      ))}
      ...
      <div style={{
        ...
        color: "#5c5550", lineHeight: 1.6,
      }}>1 kayak · 1 SUP · 2 goggles · speakers · board games · kimonos · kite hats · sunglasses · kitchen scale</div>

      <DoubleRule />

      <div style={{
        textAlign: "center", fontFamily: "'Crimson Pro', Georgia, serif",
        fontSize: 11, fontStyle: "italic", color: "#9a928a",
      }}>Clarity in complexity. Confidence in conviction.</div>
    </div>
  );
}
...
function TeamCard({ team, storageKey }) {
  const [checked, setChecked] = usePersistedState(storageKey, {});
  const toggle = (id) => setChecked((p) => ({ ...p, [id]: !p[id] }));

  const earned = CHALLENGES.reduce((s, c) =>
    s + c.items.reduce((a, i) => a + (checked[i.id] ? i.pts : 0), 0), 0);
  ...
  return (
    <div>
      ...
      <div style={{ textAlign: "center", marginBottom: 12 }}>
        <h2 style={{
          fontFamily: "'Crimson Pro', Georgia, serif",
          fontSize: 22, fontWeight: 700, color: team.accent,
          margin: "0 0 2px", letterSpacing: 1,
        }}>{team.name}</h2>
        <div style={{
          fontFamily: "'Inter', sans-serif",
          fontSize: 13, color: "#5c5550",
        }}>{team.members.join(" & ")}</div>
      </div>

      {/* Score */}
      <div style={{
        display: "flex", justifyContent: "center", gap: 24,
        padding: "10px 0", borderTop: `2px solid ${team.accent}`,
        borderBottom: "1px solid #d8d0c8", marginBottom: 14,
      }}>
        <div style={{ textAlign: "center" }}>
          <div style={{
            fontFamily: "'IBM Plex Mono', monospace",
            fontSize: 26, fontWeight: 700, color: team.accent,
          }}>{earned}</div>
          <div style={{
            fontFamily: "'Crimson Pro', Georgia, serif",
            fontSize: 9, fontStyle: "italic", color: "#9a928a",
        ...
        <div style={{ textAlign: "center" }}>
          <div style={{
            fontFamily: "'IBM Plex Mono', monospace",
            fontSize: 26, fontWeight: 700, color: "#2a2522",
          }}>{done}</div>
          <div style={{
            fontFamily: "'Crimson Pro', Georgia, serif",
            fontSize: 9, fontStyle: "italic", color: "#9a928a",
        ...
        <div key={cat.category} style={{ marginBottom: 12 }}>
          <div style={{
            ...
            <h3 style={{
              fontFamily: "'Crimson Pro', Georgia, serif",
              fontSize: 11, fontWeight: 600, textTransform: "uppercase",
              letterSpacing: 2, color: "#2a2522", margin: 0,
            }}>{cat.category}</h3>
          </div>
          ...
          {cat.items.map((item) => {
            ...
            return (
              <div
                ...
                style={{
                  display: "flex", gap: 7, padding: "4px 4px",
                  borderRadius: 2, cursor: "pointer",
                  background: isDone ? `${team.accent}0a` : "transparent",
              ...
              >
                <div style={{
                  ...
                  borderRadius: 2,
                  border: isDone ? `2px solid ${team.accent}` : "1.5px solid #c8c0b8",
                  background: isDone ? team.accent : "transparent",
                  display: "flex", alignItems: "center", justifyContent: "center",
                  fontSize: 10, color: "#faf8f4", fontWeight: 700,
                }}>
                  {isDone && "✓"}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{
                    ...
                    lineHeight: 1.3, color: isDone ? "#9a928a" : "#2a2522",
      ...
      {/* Sticky Footer */}
      <div style={{
        position: "sticky", bottom: 0, background: "#f5f1ea",
        borderTop: "2px solid #d8d0c8", padding: "8px 4px",
        ...
        <span style={{
          fontFamily: "'IBM Plex Mono', monospace",
          fontSize: 18, fontWeight: 700, color: team.accent,
        }}>{earned} pts</span>
      </div>
    </div>
  );
...
export default function App() {
  const [tab, setTab] = usePersistedState("hunt_tab", "rules");
  ...
  return (
    <div style={{
      ...
      minHeight: "100vh", background: "#f5f1ea", color: "#2a2522",
    }}>
      <link href="https://fonts.googleapis.com/css2?family=Crimson+Pro:ital,wght@0,400;0,600;0,700;1,400;1,500&family=IBM+Plex+Mono:wght@400;500;600;700&family=Inter:wght@400;500;600&display=swap" rel="stylesheet" />
      ...
      <div style={{ borderBottom: "1px solid #d8d0c8", background: "#faf8f4" }}>
        <div style={{ textAlign: "center", padding: "12px 16px 4px" }}>
          <h1 style={{
            fontFamily: "'Crimson Pro', Georgia, serif",
            fontSize: 22, fontWeight: 700, color: "#2a2522",
            letterSpacing: 1, margin: 0, lineHeight: 1.1,
          }}>The Aruba Scavenger Hunt</h1>
          <div style={{
            fontFamily: "'Crimson Pro', Georgia, serif",
            fontSize: 10, fontStyle: "italic", color: "#9a928a",
            marginTop: 2, marginBottom: 6,
          }}>Savaneta Beach · Imgesu's Birthday Edition · 17:00–19:00</div>
        </div>

        <div style={{
          display: "flex", borderTop: "1px solid #e8e2da",
        }}>
          {tabs.map((t) => {
