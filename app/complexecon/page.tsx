      ...
      <div className="flex gap-2.5">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
            ...
            <span
              className={`rounded-sm border px-1.5 py-0.5 font-mono text-[14px] uppercase tracking-[0.5px] ${TIER_CLASS[item.tier]}`}
            >
              {TIER_LABEL[item.tier]}
            </span>
            {item.kind === 'paper' && (
              <span className="rounded-sm border border-rule px-1.5 py-0.5 font-mono text-[14px] uppercase tracking-[0.5px] text-ink-muted">
                Paper
              </span>
            )}
          </div>
          <p className="mt-0.5 text-[18px] leading-relaxed text-ink-muted">{item.note}</p>
          ...
          <div className="mt-1.5 flex flex-wrap items-center gap-x-3 gap-y-1">
            {item.readerUrl && (
              <button
                ...
                className="rounded-sm border border-burgundy bg-burgundy px-2 py-0.5 font-serif text-[16px] font-medium text-paper transition-colors hover:bg-burgundy/90"
              >
            ...
            {item.sources.map(s => (
              <a
                ...
                className="font-mono text-[15px] uppercase tracking-[0.5px] text-ink-muted underline decoration-rule underline-offset-2 transition-colors hover:text-burgundy hover:decoration-burgundy/40"
              >
            ...
            <button
              ...
              className={`ml-auto rounded-sm border px-2 py-0.5 font-serif text-[16px] font-medium transition-colors ${
                memoOpen
              ...
              {signedIn ? (
                <>
                  <textarea
                    ...
                    className="w-full rounded-sm border border-rule bg-paper px-2.5 py-2 font-serif text-[20px] leading-relaxed text-ink placeholder:text-ink-faint focus:border-ink-faint focus:outline-none"
                  />
                  <div className="mt-1 flex items-center justify-between">
                    <span className="font-mono text-[14px] uppercase tracking-[1px] text-ink-faint">
                ...
                <div className="flex items-center gap-3 py-1">
                  <span className="text-[18px] text-ink-muted">Sign in to keep memos on this book.</span>
                  <button
                    onClick={onSignIn}
                    className="rounded-sm border border-burgundy bg-burgundy px-2.5 py-1 font-serif text-[16px] font-medium text-paper hover:bg-burgundy/90"
                  >
                    Sign in with Google
                  </button>
                </div>
    ...
    <main className="min-h-screen text-ink" style={{ background: '#f5f1ea' }}>
      <div className="mx-auto max-w-4xl px-4 py-10 md:px-6 md:py-14">
        ...
        <header className="mb-10 text-center">
          ...
          <p className="mt-1 font-mono text-[16px] uppercase tracking-[1.5px] text-ink-muted">
        ...
        <section className="mb-12 border-y border-rule py-8 text-center">
          <p className="font-serif text-[32px] italic leading-snug text-ink md:text-[36px]">
        ...
        <section className="mb-12">
          ...
          <p className="mb-5 font-serif text-[24px] leading-relaxed text-ink">{LANE_STATEMENT}</p>
          <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
            <div className="rounded-sm border border-rule bg-white p-3">
              ...
              <p className="text-[18px] leading-relaxed text-ink-light">
        ...
        <section className="mb-12 grid grid-cols-3 gap-3">
          <div className="rounded-sm border border-rule bg-white p-3 text-center">
            <div className="font-mono text-[30px] font-semibold text-burgundy">{weeksOut}</div>
            <div className="font-mono text-[15px] uppercase tracking-[1px] text-ink-muted">Weeks to Abu Dhabi</div>
          </div>
          <div className="rounded-sm border border-rule bg-white p-3 text-center">
            <div className="font-mono text-[30px] font-semibold text-ink">
              {loaded ? milestonesDone : '·'}<span className="text-ink-faint">/{allMilestoneIds.length}</span>
            </div>
            <div className="font-mono text-[15px] uppercase tracking-[1px] text-ink-muted">Milestones</div>
          </div>
          <div className="rounded-sm border border-rule bg-white p-3 text-center">
            <div className="font-mono text-[30px] font-semibold text-ink">
              {loaded ? booksDone : '·'}<span className="text-ink-faint">/{allBookIds.length}</span>
            </div>
            <div className="font-mono text-[15px] uppercase tracking-[1px] text-ink-muted">Library</div>
          </div>
        </section>
                ...
                <div key={stage.id} className="rounded-sm border border-rule bg-white p-4">
                  <div className="mb-1 flex items-baseline justify-between gap-2">
                    <div className="flex items-baseline gap-2.5">
                      <span className="font-serif text-[24px] font-semibold text-ink-faint">{stage.numeral}</span>
                      <h3 className="font-serif text-[24px] font-semibold text-ink">{stage.name}</h3>
                    </div>
                    <span className="font-mono text-[15px] uppercase tracking-[1px] text-ink-muted">
                      {stage.window} · {doneCount}/{total}
                    </span>
                  </div>
                  <p className="mb-3 text-[18px] leading-relaxed text-ink-light">{stage.aim}</p>
                  <div className="mb-3">
                      ...
                      return (
                        <li key={m.id} className="flex gap-2.5">
                          ...
                          <div>
                            ...
                            <p className="mt-0.5 text-[18px] leading-relaxed text-ink-muted">{m.detail}</p>
                          </div>
                        </li>
                      )
                    })}
        ...
        <section className="mb-12">
          ...
          <p className="mb-2 text-[18px] leading-relaxed text-ink-light">
            ...
            {LIBRARY.map(topic => (
              <div key={topic.id}>
                <div className="mb-1 flex items-baseline justify-between gap-2 border-b border-rule pb-1">
                  <h3 className="font-serif text-[21px] font-semibold uppercase tracking-[0.5px] text-burgundy">
                    {topic.name}
                  </h3>
                ...
                <p className="mb-2.5 text-[16px] italic leading-relaxed text-ink-muted">{topic.rationale}</p>
        ...
        <section className="mb-12">
          ...
          <p className="mb-4 text-[18px] leading-relaxed text-ink-light">
            ...
            {HYPOTHESES.map(h => (
              <div key={h.id} className="rounded-sm border border-rule bg-white p-3">
                <div className="flex gap-3">
                  <span className="font-mono text-[21px] font-semibold text-burgundy">{h.id}</span>
                  <div>
                    <p className="text-[20px] font-semibold leading-relaxed text-ink">{h.claim}</p>
                    <p className="mt-1.5 text-[18px] leading-relaxed text-ink-muted">
                      <span className="font-mono text-[15px] uppercase tracking-[1px] text-amber-ink">Test · </span>
                      {h.test}
                    </p>
                  </div>
                </div>
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
            {ARTIFACTS.map(a => (
              <div key={a.id} className="rounded-sm border border-rule bg-white p-3">
                <div className="mb-1.5 font-serif text-[20px] font-semibold uppercase tracking-[0.5px] text-burgundy">
                  {a.name}
                </div>
                <p className="text-[18px] leading-relaxed text-ink-light">{a.detail}</p>
              </div>
            ))}
          </div>
        </section>
