...
export default function PeakStatePage() {
  ...
  const inputClasses = 'w-full bg-peak-surface border border-peak-rule rounded-sm px-3 py-2 text-[12px] text-peak-paper placeholder:text-peak-ink-faint focus:outline-none focus:border-peak-gold transition-colors'

  return (
    <div className="min-h-screen bg-peak-cream">
      {/* Hero */}
      <header className="relative overflow-hidden bg-black pt-16 pb-20 px-6 text-center border-b border-peak-rule">
        ...
        <div className="relative max-w-2xl mx-auto">
          <p className="font-mono text-[10px] uppercase tracking-[3px] text-peak-teal mb-4">
            Annual Birthday Adventure · Est. 2026
          </p>
          <h1 className="font-serif text-[42px] leading-[1.1] font-semibold text-peak-gold tracking-[-0.5px] mb-3">
            Peak State
          </h1>
          <div className="w-16 h-[2px] bg-peak-gold mx-auto mb-4" />
          <p className="font-serif text-[18px] text-peak-ink-faint leading-relaxed max-w-lg mx-auto">
            ...
            <br />
            <span className="text-peak-gold">10 days. 9 towns. 1 birthday.</span>
          </p>
        </div>
      </header>
      ...
      <main className="max-w-3xl mx-auto px-4 py-10">
        ...
        <section className="mb-10" id="rsvp">
          <div className="bg-peak-surface border border-peak-rule rounded-sm p-5">
            <h2 className="font-serif text-[13px] font-semibold uppercase tracking-[0.5px] text-peak-gold mb-1 pb-2 border-b-2 border-peak-rule">
              RSVP
            </h2>
            ...
            {submitted ? (
              <div className="text-center py-8">
                ...
                <p className="font-serif text-[16px] text-peak-gold font-semibold mb-1">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div>
                    <label className="font-mono text-[10px] uppercase tracking-[1px] text-peak-ink-muted block mb-1">Name</label>
                    <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="First name" required className={inputClasses} />
                  </div>
                  <div>
                    <label className="font-mono text-[10px] uppercase tracking-[1px] text-peak-ink-muted block mb-1">Email</label>
                    <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@email.com" className={inputClasses} />
                  </div>
                  <div>
                    <label className="font-mono text-[10px] uppercase tracking-[1px] text-peak-ink-muted block mb-1">Phone</label>
                    <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+1 (555) 000-0000" className={inputClasses} />
                  </div>
                </div>

                <div>
                  <label className="font-mono text-[10px] uppercase tracking-[1px] text-peak-ink-muted block mb-2">Can you make it?</label>
                    ...
                    {([
                      ...
                      <button
                        ...
                        className={`flex-1 py-2.5 px-2 rounded-sm border text-[11px] font-medium transition-all ${
                          rsvp === option.value
                            ? option.value === 'yes'
                              ? 'bg-peak-teal text-black border-peak-teal'
                              : option.value === 'maybe'
                              ? 'bg-peak-gold text-black border-peak-gold'
                              : 'bg-peak-ink-muted text-black border-peak-ink-muted'
                            : 'bg-peak-cream text-peak-ink-muted border-peak-rule hover:border-peak-ink-faint'
                        }`}
                      >
                        <span className="flex justify-center mb-0.5">
                          {Icon[option.iconKey](rsvp === option.value ? '#0f0f0f' : undefined)}
                        </span>
                        {option.label}
                      </button>
                    ))}
                ...
                <button
                  ...
                  className="w-full py-2.5 rounded-sm font-serif text-[13px] font-semibold uppercase tracking-[0.5px] transition-all disabled:opacity-40 disabled:cursor-not-allowed bg-peak-gold text-black hover:opacity-90"
                >
        ...
        <section className="mb-10">
          <div className="bg-peak-surface border border-peak-rule rounded-sm p-5">
            <h2 className="font-serif text-[13px] font-semibold uppercase tracking-[0.5px] text-peak-gold mb-3 pb-2 border-b-2 border-peak-rule">
              Trip Details
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {DETAILS.map((d) => (
                <div key={d.label} className="flex gap-2">
                  ...
                  <span className="text-[12px] text-peak-paper font-medium leading-snug">
                    {d.value}
                  </span>
                </div>
              ))}
          ...
          <div className="bg-peak-surface border border-peak-rule rounded-sm p-5">
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                <div key={block.label} className="text-center p-3 bg-peak-cream rounded-sm border border-peak-rule-light">
                  <p className="font-mono text-[10px] text-peak-ink-muted uppercase tracking-[1px] mb-1">{block.label}</p>
                  <div className="flex justify-center mb-1">{block.icon()}</div>
                  <p className="text-[11px] text-peak-paper font-medium">{block.time}</p>
                  <p className="text-[10px] text-peak-ink-muted mt-0.5">{block.desc}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="mb-10">
          <div className="bg-peak-surface border border-peak-rule rounded-sm p-5">
            ...
            <div className="space-y-0">
              {ITINERARY.map((day, i) => (
                <div
                  ...
                  } ${day.highlight ? 'bg-peak-gold-light -mx-2 px-2 rounded-sm' : ''}`}
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-baseline gap-2 mb-0.5">
                      <span className={`font-serif text-[12px] font-semibold ${
                        day.highlight ? 'text-peak-gold' : 'text-peak-paper'
                      }`}>
                        {day.label}
                      </span>
                      <span className="font-mono text-[10px] text-peak-ink-muted">{day.date}</span>
                      {day.miles !== null && (
                        <span className="font-mono text-[10px] text-peak-teal ml-auto shrink-0">
            ...
            <div className="mt-3 pt-3 border-t-2 border-peak-rule flex justify-between items-center">
              <span className="font-mono text-[10px] uppercase tracking-[1px] text-peak-ink-muted">Total riding distance</span>
              <span className="font-mono text-[13px] font-semibold text-peak-teal">~147 mi</span>
            </div>
          </div>
        </section>
            ...
            <div className="space-y-2.5">
              {LOGISTICS.map((item) => (
                <div key={item.title} className="flex gap-2.5">
                  ...
                  <div>
                    <p className="text-[11px] font-semibold text-peak-paper mb-0.5">{item.title}</p>
        ...
        <section className="mb-10">
          <div className="bg-peak-surface border border-peak-rule rounded-sm p-5">
            ...
            <p className="text-[10px] text-peak-ink-muted mb-3">
              ...
              {COST_BREAKDOWN.map((row, i) => (
                <div
                  ...
                  className={`flex items-start gap-3 py-2 ${
                    i < COST_BREAKDOWN.length - 1 ? 'border-b border-peak-rule-light' : ''
                  }`}
                >
                  <div className="flex-1 min-w-0">
                    <p className="text-[11px] font-medium text-peak-paper">{row.item}</p>
                    <p className="text-[10px] text-peak-ink-muted">{row.note}</p>
                  </div>
                  <span className="font-mono text-[11px] text-peak-teal font-medium shrink-0">
                    ${row.low}–{row.high}
                  </span>
                </div>
              ))}
            ...
            <div className="mt-3 pt-3 border-t-2 border-peak-rule flex justify-between items-center">
              <span className="font-mono text-[10px] uppercase tracking-[1px] text-peak-ink-muted">Estimated total</span>
              <span className="font-mono text-[13px] font-semibold text-peak-gold">
